...
def create_map(gases):
    ...
    ponderadores = [0.06,0.272,2.73]
    df_fuel_emissions["total_fuel_emissions"] = (df_fuel_emissions[gases] * ponderadores).sum(axis=6)
    ...
    with rasterio.open(input_raster) as src:
        raster_data = src.read(6)  # Leer la banda 6
    ...
    with rasterio.open(output_raster, "w", **profile) as dst:
        dst.write(fuel_load_raster, 6)

    print(f"Raster único generado con la suma de los gases: {output_raster}")

def generate_fc_map():
    # 6. Read the CSV containing (fuel code -> fuel load)
    ...
    with rasterio.open(input_raster) as src:
        raster_data = src.read(6)      # Read the first band as a NumPy array
...
def sum_raster(raster6,raster2,output_raster):
    with rasterio.open(raster6) as src:
        data6 = src.read(6)  # Read the first band (assuming single-band raster)

    with rasterio.open(raster2) as src:
        data2 = src.read(6)  # Read the first band (assuming single-band raster)

    data3 = data6+data2
    header,_ = read_asc(raster6)
...
#create_map(gases)
forest = "sub40"
raster6 = f"/home/matias/Documents/Emisiones/{forest}/forest/{forest}_gf.asc"
...
#multiply_rasters(raster6,raster2,raster3)

raster6 = f"/home/matias/Documents/Emisiones/{forest}/forest/{forest}_fe_copa.asc" 
...
raster6 = f"/home/matias/Documents/Emisiones/{forest}/results/preset/{forest}_mean_sfb.asc"
...
divide_rasters(raster6,raster2,output3,raster_add)

raster6 = f"/home/matias/Documents/Emisiones/{forest}/forest/{forest}_fe.asc"
...
sum_raster(raster6,raster2,output_raster)
